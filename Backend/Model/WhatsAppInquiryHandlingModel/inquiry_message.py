...
class InquiryChatMessage(BaseModel):
    ...
    whatsapp_connection_manager.py's dispatch rule (`_handle_message`), which
    excludes every group chat from Inquiry unconditionally, on every
    connection. Every downstream step (buffering, LLM classification,
    client-record lookup) keys strictly off `sender_phone`, so a message with
