...
class EmbeddedProperty(StructuredProperty):
    ...
    RETRIEVAL (Service/property_vector_store.py) and the exact vector later
    ...
    (society_name/address/area_name — see Service/embedding_service.py's
    FIELD_EMBEDDING_NAMES), computed once at the same time, so the
    field-level duplicate comparison (Service/duplicate_detection_service.py)
