...
} from "../components/MultiAssignDialog";
...
type AssignFlow =
  | { step: "pick" }
  | { step: "handoff"; assignments: AgentAssignment[] }
  | null;
...
export default function ClientMatchesPage() {
  ...
  const [selectedMatch, setSelectedMatch] = useState<MatchedProperty | null>(
    null,
  );

  const { status: appStatus } = useAppStatus();
  const [viewingManualProperty, setViewingManualProperty] =
  ...
  const load = useCallback(async () => {
    try {
      const [matchResult, clientRecord, agentList, manualIds] =
        await Promise.all([
  ...
  async function handleRemoveManualProperty(recordId: string) {
    ...
    try {
      const updated = await inquiryClientApi.removeManualProperty(
        phone,
        recordId,
      );
  ...
  async function handleMoveToMain(match: MatchedProperty) {
    ...
    try {
      await propertyApi.updateProperty(match.record_id, {
      ...
      setResult((prev) =>
        prev ? { ...prev, ...remapCategory(prev, match.record_id) } : prev,
      );
      setSelectedMatch((prev) =>
        prev && prev.record_id === match.record_id
          ? { ...prev, property_category: "main" }
          : prev,
      );
      toast.push({
        tone: "ok",
        title: "Moved to Main",
        message: match.society_name ?? match.area_name ?? undefined,
      });
    } catch (err) {
      toast.push({
        tone: "bad",
  ...
  const manualProperties = useMemo(
    () =>
      (manualPropertyIds ?? [])
        ...
        .filter((p): p is PropertyRecord => p !== undefined),
  ...
  const selectedProperties = useMemo<SelectableProperty[]>(() => {
    ...
    for (const match of [
      ...
      if (selectedIds.has(match.record_id))
        items.push({ property: match, source: "matched" });
    }
    for (const property of manualProperties) {
      if (selectedIds.has(property.record_id))
        items.push({ property, source: "manual" });
    }
    return items;
    ...
    <div className="stack stack-5">
      <header className="section-head">
        ...
        <div className="row-flex">
          ...
          <Button
            ...
            onClick={() =>
              navigate(
                `/select-property?forClient=${encodeURIComponent(phone)}&clientName=${encodeURIComponent(client?.name || phone)}`,
              )
            }
            disabled={!client}
          >
      ...
      {(manualProperties.length > 0 || selectedIds.size > 0) && (
        <Note tone="info" icon={<IconUserCheck size={16} />}>
          Tick the properties you want to hand off — from Matches below and/or
          Manually Added — then <strong>Assign &amp; send</strong>. Different
          properties can go to different agents.
        </Note>
      )}
          ...
          <div className="stack stack-3">
            {manualProperties.map((property) => (
              <ManualPropertyRow
                ...
                assignedAgent={
                  assignedAgentByProperty.get(property.record_id) ?? null
                }
          ...
          result.low.length === 0 ? (
            ...
            <>
              <MatchSection
                ...
                assignedAgentByProperty={assignedAgentByProperty}
              />
              <MatchSection
                title="Medium match"
                bucket="medium"
                matches={result.medium}
                onMoveToMain={handleMoveToMain}
                movingId={movingId}
                onSelect={setSelectedMatch}
                selectedIds={selectedIds}
      ...
      {viewingManualProperty && (
        <ManualPropertyDetailDialog
          property={viewingManualProperty}
          onClose={() => setViewingManualProperty(null)}
        />
      )}
      ...
      {assignFlow?.step === "pick" && client && (
        <MultiAssignDialog
          ...
          onContinue={(assignments) =>
            setAssignFlow({ step: "handoff", assignments })
          }
        />
      )}
...
function remapCategory(
  ...
  recordId: string,
): Partial<ClientMatchResult> {
  const patch = (list: MatchedProperty[]) =>
    list.map((m) =>
      m.record_id === recordId
        ? { ...m, property_category: "main" as const }
        : m,
    );
  return {
      ...
      >
        <div className="stack stack-1">
          ...
          >
            ...
            <Badge tone={match.property_category === "main" ? "ok" : "info"}>
              {CATEGORY_LABEL[match.property_category]}
            </Badge>
            {assignedAgent && (
              <Badge tone="ok">Assigned to {assignedAgent.name}</Badge>
            )}
          </div>
          <div className="faint small row-flex" style={{ gap: 10 }}>
            ...
            {match.carpet_area_sqft !== null && (
              <span>
                {formatCarpetArea(
                  match.carpet_area_sqft,
                  match.carpet_area_unit,
                )}
              </span>
            )}
...
function MatchDetailDialog({
  ...
  const title =
    ...
    "Property";
  const subtitle = [
    property?.area_name ?? match.area_name,
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Matched property details"
      >
        <div className="detail-modal__head">
          <div style={{ minWidth: 0 }}>
            <div className="detail-modal__eyebrow">
              {(property?.property_type ?? match.property_type) || "Property"} ·{" "}
              {property?.listing_type ?? match.listing_type}
            </div>
            ...
            <div className="detail-modal__badges">
              ...
              {(property?.carpet_area_sqft ?? match.carpet_area_sqft) !==
                null && (
                <span className="fact">
                  <IconRuler size={12} />
                  {formatCarpetArea(
                    property?.carpet_area_sqft ?? match.carpet_area_sqft,
                    property?.carpet_area_unit ?? match.carpet_area_unit,
                  )}
                </span>
              )}
            </div>
          ...
          <div className="detail__grid">
            <div className="detail__block">
              ...
              <div className="detail__v">
                {property?.price_text ?? match.price_text ?? "—"}
              </div>
              {(property?.price_amount_inr ?? match.price_amount_inr) !==
                null && (
            ...
            <div className="detail__block">
              ...
              <div className="detail__v">
                {property?.contact_name ?? match.contact_name ?? "—"}
              </div>
              {(property?.contact_phone ?? match.contact_phone) && (
                <div className="detail__v" style={{ marginTop: 4 }}>
                  <Copyable
                    text={
                      (property?.contact_phone ?? match.contact_phone) as string
                    }
                  />
                </div>
              )}
 ...
 */
function ManualPropertyDetailDialog({
  property,
  onClose,
}: {
  property: PropertyRecord;
  ...
  const subtitle = [property.area_name, property.address]
    .filter(Boolean)
    .join(" · ");
        ...
        <div className="detail-modal__body">
          ...
          <div className="detail__grid">
            ...
            <div className="detail__block">
              ...
              <div className="detail__v">
                {property.price_per_unit_text ??
                  formatPricePerUnit(null, property.price_per_unit_amount_inr)}
              </div>
            </div>
